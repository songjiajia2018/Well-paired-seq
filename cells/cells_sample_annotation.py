...
parser = argparse.ArgumentParser(description='生成细胞注释文件，从DGE中抽取cell barcode，并添加指定的样本注释\
                                              （多个DGE的注释会在原注释文件后面补充，不覆盖原注释文件）：\
                                              python cells_sample_annotation.py \
                                              -d DGE_file \
                                              -s sample_name \
                                              -a sample_annotation')
...
try:
    f = open('cells_sample_annotation.txt', 'r')
    f.close()
...
cont_a = ''
if check_file == 1:
    file_a = open('cells_sample_annotation.txt', 'r')
    cont_a = file_a.read()
    file_a.close()
file_a = open('cells_sample_annotation.txt', 'w')
file_a.write(cont_a)
