...
## read.table(tsne_result_group_annotation.txt)
  ## file_url
data <- read.table(file = "pca_result_sample_annotation.txt",  header = T, row.names= 1 )

## Scatter plot
PC_1 <- data$PC_1
PC_2 <- data$PC_2
SAMPLE_NAME <- data$SAMPLE_NAME
data2 <- data.frame(pc_1 = PC_1, pc_2 = PC_2, sample_name = SAMPLE_NAME)

pdf(file = "pca_sample.pdf")
ggplot(data=data2,aes(x=pc_1,y=pc_2,fill=sample_name,col=sample_name))+geom_point(size=0.5)+theme_bw()+theme(panel.border=element_blank(),panel.grid.major=element_blank(),panel.grid.minor=element_blank(),axis.line=element_line(color="black"))
dev.off()
