...
diff_test_res <- differentialGeneTest(cds, fullModelFormulaStr = "~drug_conc")
ordering_genes <- row.names(subset(diff_test_res, qval < 0.01))
cds <- setOrderingFilter(cds, ordering_genes)
cds <- reduceDimension(cds, max_components = 2, method = 'DDRTree')
cds <- orderCells(cds)
pdf(file = "pseudotime_dGT_sample.pdf")
plot_cell_trajectory(cds, color_by = "drug_conc")
dev.off()
saveRDS(cds, file = "pseudotime_dGT_sample.rds")
write.table(pData(cds), file = 'pseudotime_pData_sample.txt', col.names = T, row.names = T)
